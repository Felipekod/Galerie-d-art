...
namespace Galerie
{
    class Program_bkp
    {
        ...
        static void Mains(string[] args)
        {
            ...
            LoremIpsum_Artist(15);
            LoremIpsum_Oeuvres(30);
            ...
            for (int i = 0; i < artisteCode.Count; i++)
            {
                ...
                for (int j = 0; j < oeuvreCode.Count; j++)
                {
                    if (oeuvreArtisteCode[j] == artisteCode[i])
                    {
                        ...
                        Console.WriteLine("Valeur: " + oeuvreValeur[j]);
                        if (oeuvreEtat[j] == "V")
                        {
                            Console.WriteLine("Valeur de vente: " + oeuvreValeurVente[j]);
                        }

                        oeuvreCode.RemoveAt(j);
                    }
                }
